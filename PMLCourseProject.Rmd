...
Model 2: 
The second model simply reduces ntree to 100, which reduces the run time to ~ 30 - 40 seconds (same machine and set-up as for Model 1).  Out-of-sample error estimate (OOB estimate of error rate) is 0.82% (99+% accuracy), but mtry=27, meaning a different sampling at each split was chosen as the final model (based on accuracy).  
